  ...
  <div class="index-tpl">
      ...
      <draggable
        ...
        <div v-for="(item,index) in content.list" :style="{marginTop:item.top+'px',marginBottom:item.bottom+'px',paddingLeft:item.left+'px',paddingRight:item.right+'px', backgroundColor:item.bgcolor,backgroundImage: 'url('+ item.bgurl +')',backgroundSize:'cover',backgroundRepeat: 'no-repeat', borderRadius:item.borderRadius+'px'}" :key="index" class="tpl-item">
          <div class="tpl-item-box">
            <div v-if="item.type!=2&&item.type!=4&&item.type!=10&&item.type!=17&&item.type!=19&&item.type!=20&&item.type!=21&&item.type!=22" class="tpl-item-box1">
            ...
            <div v-if="item.type==21" class="tpl-item-box1">
              <div class="item-box-item">
                <div style="display: flex;">
                  <div style="width:80px" v-for="(item2,index2) in item.origList" :key="index2">{{item2.name}}&nbsp;&nbsp;&nbsp;</div>
                </div>
              </div>
            </div>
            ...
            <el-popover
              ...
              <div class="edit-panel">
                ...
                <div class="one-row" style="position: relative;">
                  ...
                  <div v-if="item.type==22" class="row-col" style="flex:none;margin-bottom:10px;position: absolute;top: 80px;">
                    <span>选择模块：</span>
                    <el-select v-model="item.listNums" style="width:150px;padding-right:15px;" size="mini" placeholder="请选择配置项" @change="chooseMukuai1(index,item.listNums)">
                ...
                <div style="max-height: 610px;overflow-y: auto;">
                  ...
                  <div v-if="item.type!=1&&item.type!=6&&item.type!=7&&item.type!=8&&item.type!=9&&item.type!=10&&item.type!=17&&item.type!=11&&item.type!=12&&item.type!=13&&item.type!=14&&item.type!=15&&item.type!=16&&item.type!=19&&item.type!=21&&item.type!=22" class="two-row">
                  ...
                  <div v-if="item.type==21">
                    <div>
                      <!-- <el-select v-model="class_name" placeholder="请选择分类" @change="allSelectOne1(index)">
                        <el-option
                      ...
                      <el-button type="primary" @click="addCory">选择商品分类</el-button>
                      <span style="color:red;">（* 选择分类导航栏）</span>
                    ...
                    <div style="padding:20px 0">
                      <el-tag
                        style="margin:5px"
                        v-for="(tag,index2) in item.origList"
                        :key="index2"
                        closable
                        @close="delClassItem(index,index2)">
                        {{tag.name}}
                      </el-tag>
                      <!-- <div style="padding:20px 0" v-for="(item2,index2) in item.origList" :key="index2">{{item2.name}}&nbsp;&nbsp;&nbsp;<el-button type="danger" icon="el-icon-delete" size="mini" circle @click="delClassItem(index,index2)"></el-button> </div> -->
                    </div>
                    <!-- &nbsp;&nbsp;&nbsp;
                  ...
                  <div v-if="item.type==19">
                    ...
                    <div v-for="(item3,index3) in item.moduleList" :key="index3" style="border:1px solid #ddd;padding: 10px;margin: 10px 0;position: relative;">
                      ...
                      <el-button @click="deletaMokuia(index,index3)" type="danger" round style="position: absolute;right:10px;bottom:8px;z-index: 11;">删除模块{{item3.type}}</el-button>
                    </div>
                  </div>
                  ...
                  <div v-if="item.type==22">
                    <div v-for="(item3,index3) in item.moduleList" :key="index3+'i'" style="border:1px solid #ddd;padding: 10px;margin: 10px 0;position: relative;">
                      <el-row style="line-height:32px;display:flex;" v-if="item3.title!=undefined">
                        <span style="padding-right:4px;">模块标题：</span>
                        <el-input v-model="item3.title" style="width:350px;" size="mini" placeholder="模块标题" />
                      </el-row>
                            ...
                            <div class="data-box">
                              ...
                              <el-row style="line-height:34px;">
                                ...
                                <el-select v-model="item2.type" style="width:160px;" size="mini" placeholder="打开页面">
                                  <el-option label="无" value="0"/>
                                  <el-option label="商品" :disabled="true" value="1"/>
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="礼包商品" value="15"/> -->
                                  <el-option label="装修页" :disabled="true" value="9"/>
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="新人必买" value="10"/> -->
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="品牌精选" value="11"/> -->
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="VIP专区" value="12"/> -->
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="签到领币" value="13"/> -->
                                  <el-option label="富文本" :disabled="true" value="7"/>
                                  <el-option label="外链" :disabled="true" value="8"/>
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="分类商品列表" value="2"/> -->
                                  <el-option label="店铺" value="3"/>
                                  <!-- <el-option v-if="tplInfo.shop_id==0" label="热门商品列表" value="4"/>
                                  <el-option v-if="tplInfo.shop_id==0" label="新品推荐列表" value="5"/>
    ...
    <div class="f-tools">
      <div class="tool-box">
        ...
        <!-- <div class="tool-item">
          <el-button type="primary" style="width:100px;" size="mini" icon="el-icon-plus" @click="addTpl(22)">店铺列表</el-button>
        </div> -->
        <div class="tool-item">
    ...
    <div v-if="iscoryShow">
      <el-dialog :width="'700px'" :visible.sync="dialogTableVisible" :title="activeFlag?'选择活动商品分类':'选择活动商品分类'">
        <el-row>
          <el-col v-show="!activeFlag" :span="5">
            ...
            >
              <el-menu-item v-for="(item,index) in classList" :index="index+''" :key="index" style="padding:0 5px;height: 42px;line-height: 42px;" @click="selectClass1(item)">
          ...
          <el-col v-show="!activeFlag" :span="5" style="min-height:100px">
            <div v-if="class2.length>0" >
              <el-menu
                ...
                class="el-menu-vertical-demo"
              >
                <el-menu-item v-for="(item1,index1) in class2List" :index="index1+''" :key="index1" style="padding:0 5px;height: 42px;line-height: 42px;" @click="selectClass2(item1)">
  ...
  methods: {
    ...
    */
    ...
    allSelectOne1(index) {
      ...
      for(let i=0;i<this.classList.length;i++) {
        if(this.classList[i].mcid==this.class_name){
          this.content.list[index].origList.push(this.classList[i])
        }
      }
      this.class_name = ''
      console.log(this.content.list[index].origList)
    },
    // 选择分类
    addCory() {
    ...
    delClassItem(index,index1) {
      // console.log(index,index1)
      this.content.list[index].origList.splice(index1,1)
      // console.log(this.content.list[index].origList)
    },
    ...
    */
    ...
    saveTplInfo() {
      ...
      // console.log(JSON.stringify(self.content))
